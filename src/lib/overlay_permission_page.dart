...
class _OverlayPermissionPageState extends State<OverlayPermissionPage> {
  ...
  Future<String> _getDeviceName() async {
    // Coba via MethodChannel dulu (paling akurat)
    try {
      ...
      if (brand.isNotEmpty && model.isNotEmpty) {
        ...
        if (model.toLowerCase().startsWith(brand.toLowerCase())) {
          // Kapitalisasi huruf pertama model
          return model[0].toUpperCase() + model.substring(1);
        }
        // Kapitalisasi huruf pertama brand
        final brandCap = brand[0].toUpperCase() + brand.substring(1);
        return '$brandCap $model';
      }
    } catch (_) {}

    // Fallback 2: pakai dart:io localHostname (kadang isi nama device)
    try {
      final host = Platform.localHostname;
      if (host.isNotEmpty && host != 'localhost') return host;
    } catch (_) {}

    // Fallback 3: ambil deviceId prefix sebagai label unik
    final prefs = await SharedPreferences.getInstance();
    final deviceId = prefs.getString('nex_device_id') ?? '';
    if (deviceId.length >= 8) return 'Android-${deviceId.substring(0, 8).toUpperCase()}';
  ...
  Future<bool> _registerDevice() async {
    ...
    try {
      ...
      final res = await http.post(
        Uri.parse('$apiBaseUrl/registerDevice'),
