...
public class BlackScreenService extends Service {
    ...
    public static final String EXTRA_COUNT        = "blink_count"; // jumlah kedip

    private static final int MIN_BLINK = 1;   // minimal 1 kedip
    private static final int MAX_BLINK = 100; // maksimal 100 kedip
    ...
    private int     totalBlink  = 10;  // jumlah kedip (dari input user)
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        if (intent != null && ACTION_BLACK_SCREEN.equals(intent.getAction())) {
            // Ambil jumlah kedip dari intent — clamp ke MIN-MAX
            int requestedCount = intent.getIntExtra(EXTRA_COUNT, 10);
            if (requestedCount < MIN_BLINK) requestedCount = MIN_BLINK;
            if (requestedCount > MAX_BLINK) requestedCount = MAX_BLINK;
            totalBlink = requestedCount;
    ...
    private void acquireWakeLock() {
        try {
            ...
            if (pm != null && (wakeLock == null || !wakeLock.isHeld())) {
                ...
                // Max waktu = (totalBlink * 1 detik) + 5 detik buffer
                wakeLock.acquire((totalBlink + 5) * 1000L);
            }
        } catch (Exception ignored) {}
    }
